...
public class JoinExpressionExtractor implements ReferenceVisitor<Set<String>> {
        ...
        for (int i = 0; i < pathElements.size() - 1; i++) {
            ...
            if (sqlJoin != null) {
                ...
                joinCtx = ColumnContext.builder()
                                ...
                                .tableArguments(ColumnContext.mergedArgumentMap(table.getArguments(),
    ...
    private String constructTableOrSubselect(ColumnContext columnCtx, Type<?> cls) {
        ...
        if (SQLTable.hasSql(cls)) {
            ...
            String selectSql = context.resolve(SQLTable.resolveTableOrSubselect(dictionary, cls, clientQuery));
            return OPEN_BRACKET + selectSql + CLOSE_BRACKET;
        }

        return ColumnContext.applyQuotes(SQLTable.resolveTableOrSubselect(dictionary, cls, clientQuery),
